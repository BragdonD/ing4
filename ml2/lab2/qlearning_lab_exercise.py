...
# a_t is the action to be taken at state s_t.
# r_t is the reward obtained from taking action a_t at state s_t.
# s_t1 is the next state after taking action a_t at state s_t.
# s_t is the current state.

for episode in range(n_episodes):
    # randomly choosed first state
    s_t = np.random.randint(0,12) # there are 12 states
    done = False
    
    while not done: # as long as we do not find the heaven or hell
        m = np.random.rand() # random number to decide our action
        
        if m < eps:
            a_t = np.random.randint(0,4) # there are 4 actions
        else:
            a_t = np.argmax(Q[s_t,:]) # the best action value from the Q-table for this state
            
        r_t, s_t1 = env.step(s_t, a_t) # get the reward and the next state from the environment 
        
        # update the loop information
        Q[s_t, a_t] = (1 - alpha) * Q[s_t, a_t] + alpha * (r_t +  factor_discount * np.max(Q[s_t1, :]))
        s_t = s_t1
        
        if (s_t == 7) or (s_t == 11): # if we are in the hell or heaven
            done = True
